...
module.exports = function(io, socket) {
    ...
    socket.on('chatMessage', function(message) {
	 ...
	 var monitorValue = setInterval(function(){
        ...
        var Url_M_Pressure1 = "http://127.0.0.1:3000/channels/7/feeds/last"
        data_M_Pressure1 = monitor.getMonitorData(Url_M_Pressure1);
        
        var Url_M_Temp = "http://127.0.0.1:3000/channels/8/feeds/last"
        data_M_Temp = monitor.getMonitorData(Url_M_Temp);
        
        var Url_M_Gas = "http://127.0.0.1:3000/channels/9/feeds/last"
        data_M_gas = monitor.getMonitorData(Url_M_Gas);
        
        //var dataSet = {"Url_M_ActuOpen": data_M_ActuOpen , "Url_M_ActuClose ": data_M_ActuClose }
		var dataSet = {"data_M_Pressure1" : data_M_Pressure1,"data_M_gas": data_M_gas,"data_M_Temp":data_M_Temp };
    ...
    socket.on('tmpMessage', function(message) {
        ...
        var Url_M_ActuClose = "http://127.0.0.1:3000/channels/4/feeds/last"
        data_M_ActuClose = monitor.getMonitorData(Url_M_ActuClose);
        
        var Url_C_ActuOpen = "http://127.0.0.1:3000/channels/5/feeds/last"
        data_C_ActuOpen = monitor.getMonitorData(Url_C_ActuOpen);
        
        var Url_C_ActuClose = "http://127.0.0.1:3000/channels/6/feeds/last"
	...
	socket.on('ControlMessage',function(message){
		...
		if(message.status == 'pressure'){
			var url = "http://192.168.1.41:3000/update?key=3Z6HP8IG2L1UVJ5R&field1=" + message.text;
			console.log(url);
		...
		if(message.status == 'gas'){
			var url = "http://192.168.1.41:3000/update?key=MCDBYNPP9KJQ88G4&field1=" + message.text;
			console.log(url);
		...
		if(message.status == 'tempar'){
			var url = "http://192.168.1.41:3000/update?key=M750998VWC7VCJVP&field1=" + message.text;
			console.log(url);
